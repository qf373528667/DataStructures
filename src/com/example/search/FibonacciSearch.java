...
public class FibonacciSearch {
	...
	//因为后面我们mid=low+F(k-1)-1,需要使用到斐波那切数列，因此我们需要先获取到一个斐波那契数列
	//非递归方法得到一个斐波那契数列
	...
	public static int fibSearch(int[] a,int key){
		...
		//因为f[k] 值可能大于数组a 的长度，因此我们需要使用Arrays类，构造一个新的数组，并指向a[]
		//不足的部分会使用0填充
		...
		while(low<=high){//只要这个条件满足，就可以找
			...
			if(key<temp[mid]){  //我们应该继续向数组的前面查找（左边）
				...
				//即 在 f[k-1]的前面继续查找 k--
				//即下次查找循环 mid=low+ f[k-1-1]-1
				k--;
			}else if(key>temp[mid]){  //我们应该继续向数组后面查找（右边）
				low=mid+1;
				//为什么是k-=2
				//说明
				//1.全部元素= 前面元素+后面元素
