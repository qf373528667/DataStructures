...
//		singleLinkedList.del(3);
		...
		//测试 得到倒数第K个节点
		HeroNode cur=findListIndexNode(singleLinkedList.getHead(),3 );
	...
	public static void reversetList(HeroNode head){
		...
		//遍历原来的链表，并 每遍历一个节点，就将其取出，并放在新链表reverseHead的最前端
		while(cur!=null){
...
class SingleLinkedList{
	...
	public void add(HeroNode heroNode){
		...
		//当退出while循环时，temp就是指向了链表的最后
		temp.next=heroNode;
	}
	//第二种方式添加英雄
	public void addByOrder(HeroNode heroNode){
		//因为head节点不能动，因此我们仍然需要一个辅助指针（变量）来帮助找到添加的位置temp
		...
		while(true){
			...
			if(temp.next.no>heroNode.no){  //位置找到了，就在temp的后面插入
				break;
			}else if(temp.next.no==heroNode.no){//说明希望添加的hreoNode的编号已经存在
				flag=true;
				break;
			}
